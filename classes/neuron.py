...
class Neuron:
    ...
    # """ Used in leaky mode to store attributes of all neurons.
    # In spike mode are useful to collect data for plotter
    ...
    # # CLASS METHODS #
    # @classmethod
    # def add_w(cls, weights): cls.W.append(weights)

    # @classmethod
    # def add_link(cls, connection): cls.CONNECTIONS.append(connection)

    # @classmethod
    # def leaky_synapse(cls, t):
    #     """ In leaky mode takes voltage from layer at time t-1 to compute
    #     voltage at time t. Here link[0] is the source from which to
    #     take voltage, and link[1] is the term where to put results """
    #     for index, link in enumerate(cls.CONNECTIONS):
    #         cls.V_IN[link[1]][t] += ((cls.VOLTAGE[link[0]][t - 1] +
    #                                   cls.v_reset) * cls.W[index]) - cls.v_reset
